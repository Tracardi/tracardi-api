...
# todo refactoring, change name from consent_type_endpoint to consent_endpoint
...
@router.post("/consents/id/{profile_id}", tags=["consent"], include_in_schema=server.expose_gui_api, response_model=dict)
async def add_consent(profile_id: UUID, consent: ConsentSchema):
    try:
        ...
        if profile_to_modify is None:
            raise HTTPException(status_code=500, detail="There is no profile with following id: {}".format(profile_id))
