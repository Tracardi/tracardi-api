...
async def get_field_compliance_with_customer_consent(query: Optional[str] = None, start: int = 0, limit: int = 100):
    result = await storage.driver.data_compliance.load_all(limit=limit)
    return group_records(result, query, group_by=None, search_by='event_type.name', sort_by=None)
