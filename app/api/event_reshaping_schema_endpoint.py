...
@router.delete("/event-reshape-schema", tags=["reshaping"], include_in_schema=server.expose_gui_api, response_model=dict)
async def delete_reshape_schema(id: str, event_type: str):
...
@router.get("/event-reshape-schema", tags=["reshaping"], include_in_schema=server.expose_gui_api, response_model=dict)
async def load_reshape_schemas(limit: int = 100, query: str = None):
    result, total = await storage.driver.event_reshaping.load_all(limit)
